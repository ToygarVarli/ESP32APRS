...
#ifdef ARDUINO_ARCH_ESP32
    // ESP32 için DAC init gereksiz - I2S kullanılıyor
    #define AFSK_DAC_INIT()        do { } while (0)
#else
    // AVR için DAC init
...
// ESP32 ve AVR için ortak LED kontrol makroları
// Kullanıcı isterse GPIO pinlerini tanımlayabilir
#ifdef ARDUINO_ARCH_ESP32
    // ESP32 için LED pinleri (opsiyonel, kullanıcı tanımlayabilir)
    #ifndef LED_TX_PIN
        #define LED_TX_PIN -1
    #endif
    #ifndef LED_RX_PIN
        #define LED_RX_PIN -1
    #endif
    #define LED_TX_INIT() do { if(LED_TX_PIN >= 0) pinMode(LED_TX_PIN, OUTPUT); } while (0)
    #define LED_TX_ON()   do { if(LED_TX_PIN >= 0) digitalWrite(LED_TX_PIN, HIGH); } while (0)
    #define LED_TX_OFF()  do { if(LED_TX_PIN >= 0) digitalWrite(LED_TX_PIN, LOW); } while (0)
    #define LED_RX_INIT() do { if(LED_RX_PIN >= 0) pinMode(LED_RX_PIN, OUTPUT); } while (0)
    #define LED_RX_ON()   do { if(LED_RX_PIN >= 0) digitalWrite(LED_RX_PIN, HIGH); } while (0)
    #define LED_RX_OFF()  do { if(LED_RX_PIN >= 0) digitalWrite(LED_RX_PIN, LOW); } while (0)
#else
    // AVR için LED kontrol
