...
// Runtime ayarlanabilir ADC parametreleri
int adc_center_value = ADC_CENTER_VALUE;
int adc_signal_range = ADC_SIGNAL_RANGE;
int squelch_threshold = KEEP_RECORDING_THRESH;
...
void AFSK_hw_init(void) {
    ...
    // Sürekli dinleme modu - GPIO trigger yok
    ESP_LOGI("AFSK", "Continuous RX mode enabled");
#else
    ...
    // ÖNCELİKLE I2S driver'ı kur (task başlamadan önce!)
    ...
    ESP_LOGI("AFSK", "I2S driver initialized");

    //create a queue to handle gpio event from isr
    gpio_evt_queue = xQueueCreate(10, sizeof(uint32_t));
    
#if CONTINUOUS_RX_MODE
    //start continuous receive audio task - I2S artık hazır
...
static int hdlc_flag_count = 0;  // Debug counter
...
static bool hdlcParse(Hdlc *hdlc, bool bit, FIFOBuffer *fifo) {
    ...
    if (hdlc->demodulatedBits == HDLC_FLAG) {
        hdlc_flag_count++;
        if (hdlc_flag_count <= 10) {  // İlk 10 flag'i yazdır
            Serial.printf("HDLC FLAG #%d detected!\n", hdlc_flag_count);
        }
...
void record_audio(uint16_t *buffer) {
    size_t bytes_read;
    esp_err_t error = i2s_read(TNC_I2S_NUM, (void*) buffer, TNC_I2S_BUFLEN * sizeof(uint16_t), &bytes_read, portMAX_DELAY);
    (void)error; // Kullanılmayan değişken uyarısını önle

    for (int i=0; i<TNC_I2S_BUFLEN; i++) {
        // I2S ADC formatı: Üst 12 bit veri, alt 4 bit kanal bilgisi
        // Veriyi çıkar ve polariteyi düzelt
        buffer[i] = buffer[i] & 0x0FFF;  // Sadece 12 bit ADC verisi
    }
}
...
void process_audio(uint16_t *buffer) {
    ...
    for (int i=0; i<TNC_I2S_BUFLEN; i += OVERSAMPLING) {
        ...
        // ADC center değerini çıkar (idle voltage) - runtime değişkeni kullan
        average -= adc_center_value;
        
        // -128 ile 127 arasına normalize et - runtime değişkeni kullan
        average = average * 255 / adc_signal_range;
...
void receive_audio_task(void *arg) {
    ...
    Serial.println("AFSK: Starting continuous RX mode");
    Serial.printf("AFSK: ADC Center=%d, Range=%d, Squelch=%d\n", adc_center_value, adc_signal_range, squelch_threshold);
    
    uint16_t *buffer = audio_buf1;
    bool is_receiving = false;
    int silence_count = 0;
    const int SILENCE_THRESHOLD = 3; // Kaç sessiz buffer sonrası işleme başlasın
    
    // I2S ADC'yi etkinleştir
    esp_err_t err = i2s_adc_enable(TNC_I2S_NUM);
    if (err != ESP_OK) {
        Serial.printf("AFSK: i2s_adc_enable FAILED! err=%d\n", err);
        vTaskDelete(NULL);
        return;
    }
    Serial.println("AFSK: I2S ADC enabled successfully");
    
    // İlk birkaç okumayı debug için yazdır
    int debug_count = 0;
    ...
    for(;;) {
        ...
        // Debug: İlk 5 buffer'ın ilk 10 sample'ını yazdır
        if (debug_count < 5) {
            Serial.printf("AFSK: Buffer %d samples: ", debug_count);
            for (int i=0; i<10; i++) {
        ...
        // Ses var mı kontrol et
        bool has_audio = false;
        int max_deviation = 0;
        for (int i=0; i<TNC_I2S_BUFLEN; i++) {
            int deviation = abs((int)buffer[i] - adc_center_value);
            if (deviation > max_deviation) max_deviation = deviation;
            if (deviation > squelch_threshold) {
                has_audio = true;
                break;
            }
        }
        ...
        if (has_audio) {
            // Ses var - işle ve buffer'a ekle
            if (!is_receiving) {
                Serial.printf("AFSK: Signal detected! max_dev=%d\n", max_deviation);
                is_receiving = true;
                LED_RX_ON();
            }
            silence_count = 0;
            ...
            if (is_receiving) {
                silence_count++;
                // Birkaç sessiz buffer daha işle (paketin sonunu yakala)
                process_audio(buffer);
                
                if (silence_count >= SILENCE_THRESHOLD) {
                    // Yeterince sessizlik - paketi işle
                    Serial.printf("AFSK: Signal ended, processing %d samples\n", audio_buf_full_idx);
                    is_receiving = false;
                    LED_RX_OFF();
                    
                    // Toplanan sesi decode et
                    int running_sum = 0;
                    ...
                    // Debug: HDLC flag sayısını raporla
                    extern int hdlc_flag_count;
                    Serial.printf("AFSK: Decode complete. HDLC flags found: %d\n", hdlc_flag_count);
                    hdlc_flag_count = 0;  // Reset for next packet
        ...
        // Buffer değiştir
        buffer = (buffer == audio_buf1) ? audio_buf2 : audio_buf1;
        
        // Küçük bir delay - CPU'yu rahatlat
        vTaskDelay(1);
    }
#else
    // Eski trigger tabanlı mod
    gpio_num_t io_num;
    for(;;) {
