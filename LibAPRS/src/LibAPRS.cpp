...
// AFSK.cpp'deki runtime değişkenlere erişim
extern int adc_center_value;
extern int adc_signal_range;
extern int squelch_threshold;

void APRS_setADCCenter(int center) {
    adc_center_value = center;
    Serial.print(F("ADC Center set to: ")); Serial.println(center);
}

void APRS_setADCRange(int range) {
    adc_signal_range = range;
    Serial.print(F("ADC Range set to: ")); Serial.println(range);
}

void APRS_setSquelchThreshold(int thresh) {
    squelch_threshold = thresh;
    Serial.print(F("Squelch Threshold set to: ")); Serial.println(thresh);
}

void APRS_calibrateADC(void) {
    Serial.println(F("ADC Calibration starting..."));
    Serial.println(F("Make sure there is NO audio signal for 5 seconds!"));
    
    // I2S'den birkaç saniye sessizlik oku ve ortalamasını al
    extern uint16_t audio_buf1[];
    extern void record_audio(uint16_t *buffer);
    ...
    while (millis() - start_time < 5000) {
        record_audio(audio_buf1);
        for (int i = 0; i < 1000; i++) {  // İlk 1000 sample
            int val = audio_buf1[i];
            total += val;
            samples++;
    ...
    int new_center = total / samples;
    int noise_range = max_val - min_val;
    
    // Squelch threshold = gürültü * 3 (güvenlik marjı)
    int new_squelch = noise_range * 3;
    if (new_squelch < 50) new_squelch = 50;  // Minimum değer
    
    Serial.println(F("Calibration results:"));
    Serial.print(F("  Samples: ")); Serial.println(samples);
    Serial.print(F("  Min ADC: ")); Serial.println(min_val);
    Serial.print(F("  Max ADC: ")); Serial.println(max_val);
    Serial.print(F("  Average (Center): ")); Serial.println(new_center);
    Serial.print(F("  Noise range: ")); Serial.println(noise_range);
    Serial.print(F("  Suggested squelch: ")); Serial.println(new_squelch);
    
    adc_center_value = new_center;
    squelch_threshold = new_squelch;
    
    Serial.println(F("ADC Center and Squelch updated!"));
    Serial.println(F("Note: ADC Range not changed. Set manually if needed."));
}
#endif
